...
class ModelTrainer:
    ...
    def __init__(self, model, restore_snap=None):
        ...
        if on_CPU:
            ...
            cProto = tf.ConfigProto(intra_op_parallelism_threads=1,
    ...
    def add_train_ops(self, model):
        ...
        with tf.variable_scope('optimizer'):
            ...
            # Training step op
            if hasattr(model.config, 'freeze_layers') and model.config.freeze_layers:
                new_trainable_vars = []
                for var in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='KernelPointNetwork'):
                    layer_name = var.name.split('/')[1]
                    freeze = False
                    if 'layer' in layer_name:
                        layer_ind = int(layer_name.split('_')[-1])
                        if layer_ind in model.config.freeze_layers:
                            freeze = True
                    if not freeze:
                        new_trainable_vars += [var]
                gvs = optimizer.compute_gradients(model.loss, var_list=new_trainable_vars)
        ...
        with tf.variable_scope('results'):
            ...
            if model.config.network_model in ['multi_cloud_segmentation']:

                ignored_bool = tf.zeros_like(model.labels, dtype=tf.bool)
                for d_i, dataset_ign_labels in enumerate(model.config.ignored_label_inds):
                    dataset_bool = tf.equal(model.inputs['super_labels'], d_i)
                    for ign_label in dataset_ign_labels:
                        dataset_ignore_bool = tf.logical_and(dataset_bool, tf.equal(model.labels, ign_label))
                        ignored_bool = tf.logical_or(ignored_bool, dataset_ignore_bool)
                ...
                new_super_labels = tf.gather(model.inputs['super_labels'], inds, axis=0)

                # Reduce label values in the range of logit shape
                for d_i, dataset_ign_labels in enumerate(model.config.ignored_label_inds):
                    d_bool = tf.cast(tf.equal(new_super_labels, d_i), tf.int32)
                    reducing_list = tf.range(model.config.num_classes[d_i], dtype=tf.int32)
                    inserted_value = tf.zeros((1,), dtype=tf.int32)
                    for ign_label in dataset_ign_labels:
                        reducing_list = tf.concat(
                            [reducing_list[:ign_label], inserted_value, reducing_list[ign_label:]], 0)
                    tmp_labels = new_labels * d_bool
                    correct_labels = tf.gather(reducing_list, tmp_labels)
                    new_labels = new_labels * (1 - d_bool) + correct_labels * d_bool

                # Metrics
                self.correct_prediction = tf.nn.in_top_k(new_logits, new_labels, 1)
                self.accuracy = tf.reduce_mean(tf.cast(self.correct_prediction, tf.float32))
                self.prob_logits = tf.nn.softmax(new_logits)
                self.val_logits = tf.nn.softmax(model.logits)

            elif len(model.config.ignored_label_inds) > 0:
    ...
    def train(self, model, dataset, debug_NaN=False):
        ...
        while self.training_epoch < model.config.max_epoch:
            ...
            try:
                ...
                if debug_NaN:
                    all_values = self.sess.run(ops + list(dataset.flat_inputs), {model.dropout_prob: 0.5})
                ...
                if hasattr(model.config, 'dataset'):
                ...
                elif model.config.network_model == 'multi_cloud_segmentation':
                    self.multi_cloud_validation_error(model, dataset)
                elif model.config.network_model == 'slam_segmentation':
                    self.slam_validation_error(model, dataset)
                else:

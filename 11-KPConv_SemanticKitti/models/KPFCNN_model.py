    ...
    def __init__(self, flat_inputs, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            if config.network_model in ["cloud_segmentation", 'multi_cloud_segmentation']:
                ...
                if config.network_model in ['multi_cloud_segmentation']:
                    ind += 1
                    self.inputs['dataset_inds'] = flat_inputs[ind]

            elif config.network_model in ['multi_segmentation', 'segmentation']:
                ind += 1
                self.inputs['object_inds'] = flat_inputs[ind]

            elif config.network_model in ['slam_segmentation']:
                ind += 1
                self.inputs['frame_inds'] = flat_inputs[ind]
                ind += 1
                self.inputs['frame_centers'] = flat_inputs[ind]
        ...
        with tf.variable_scope('loss'):
            ...
            if config.network_model in ["multi_segmentation"]:
                self.output_loss = multi_segmentation_loss(self.logits,
                                                           self.inputs,
                                                           batch_average=self.config.batch_averaged_loss)

            elif config.network_model in ['multi_cloud_segmentation']:

                ignored_bool = tf.zeros_like(self.labels, dtype=tf.bool)
                for d_i, dataset_ign_labels in enumerate(self.config.ignored_label_inds):
                    dataset_bool = tf.equal(self.inputs['super_labels'], d_i)
                    for ign_label in dataset_ign_labels:
                        dataset_ignore_bool = tf.logical_and(dataset_bool, tf.equal(self.labels, ign_label))
                ...
                new_dict['super_labels'] = tf.gather(self.inputs['super_labels'], inds, axis=0)

                # Reduce label values in the range of logit shape
                for d_i, dataset_ign_labels in enumerate(self.config.ignored_label_inds):
                    d_bool = tf.cast(tf.equal(new_dict['super_labels'], d_i), tf.int32)
                    reducing_list = tf.range(self.config.num_classes[d_i], dtype=tf.int32)
                    inserted_value = tf.zeros((1,), dtype=tf.int32)
                    for ign_label in dataset_ign_labels:
                        reducing_list = tf.concat(
                            [reducing_list[:ign_label], inserted_value, reducing_list[ign_label:]], 0)
                    tmp_labels = new_dict['point_labels'] * d_bool
                    correct_labels = tf.gather(reducing_list, tmp_labels)
                    new_dict['point_labels'] = new_dict['point_labels'] * (1 - d_bool) + correct_labels * d_bool

                # Add batch weigths to dict if needed
                if self.config.batch_averaged_loss:
                    new_dict['batch_weights'] = self.inputs['batch_weights']

                self.output_loss = multi_segmentation_loss(new_logits,
                                                           new_dict,
                                                           batch_average=self.config.batch_averaged_loss)

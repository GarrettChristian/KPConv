...
class Dataset:
    ...
    def calibrate_batches(self, config):
        ...
        elif 'slam' in self.network_model:
            # no batch calibration needed for slam dataset
            return 0
    ...
    def init_test_input_pipeline(self, config, on_val=False):
        """
        Prepare the input pipeline with tf.Dataset class
        """
        ...
        # Get generator and mapping function
        if on_val:
            gen_function, gen_types, gen_shapes = self.get_batch_gen('validation', config)
        else:

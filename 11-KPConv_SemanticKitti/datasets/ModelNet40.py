...
class ModelNet40Dataset(Dataset):
    ...
    def load_subsampled_clouds(self, subsampling_parameter):
        ...
        if correct_orientation:
            for split in ['training', 'validation']:
                self.input_points[split] = [pp[:, [0, 2, 1]] for pp in self.input_points[split]]
                self.input_normals[split] = [nn[:, [0, 2, 1]] for nn in self.input_normals[split]]
    ...
    def get_batch_gen(self, split, config):
        ...
        # Reset potentials
        self.potentials[split] = np.random.rand(len(self.input_labels[split])) * 0.1 + 0.1


        # Initiate parameters depending on the chosen split
        if split == 'training':

            # Balance training sample classes and use potentials?
            balanced = False
            use_potential = True
            ...
            raise ValueError('Unkown split name')
        ...
        def random_balanced_gen():
            ...
            if split == 'training':
                ...
                if config.epoch_steps and config.epoch_steps * config.batch_num < self.num_train:
                    epoch_n = config.epoch_steps * config.batch_num
                else:
                    epoch_n = self.num_train

            elif split == 'validation':

                # First compute the number of point we want to pick in each cloud and for each class
                epoch_n = min(self.num_test, config.validation_size * config.batch_num)
            ...
            if use_potential:
                ...
                if balanced:
                    ...
                    gen_indices = np.random.permutation(epoch_n)[:epoch_n]
    ...
    def get_tf_mapping(self, config):
        ...
        def tf_map(stacked_points, stacked_normals, labels, obj_inds, stack_lengths):
            ...
            if config.in_features_dim > 1:
                ...
                stacked_additional = stacked_features[:, 1:] * tf.expand_dims(stacked_s, axis=1)
                stacked_features = tf.concat((stacked_features[:, :1], stacked_additional), axis=1)

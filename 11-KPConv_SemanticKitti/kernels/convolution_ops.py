...
def KPConv_ops(query_points,
    ...
    #
    #   In TF 1.13, tf.matmul seem to give wrong result for large matrices (in our case we can have something like
    #   [100000, 15, 35] @ [100000, 35, 4])
    #
    ...
    # Debug block to show the errors
    if False and ('simple_0' in all_weights.name):

        print_op = tf.print('\n', all_weights.name,
                            '\n              all_weights  - ', tf.shape(all_weights), '  -  ',
                            'Min : ', tf.reduce_min(all_weights), ' / ',
                            'Max : ', tf.reduce_max(all_weights), ' / ',
                            'Mean : ', tf.reduce_mean(all_weights), ' / ',
                            'Num > 1e6 : ', tf.reduce_any(tf.greater(tf.abs(all_weights), 1e6)),
                            tf.reduce_sum(tf.cast(tf.greater(tf.abs(all_weights), 1e6), tf.int32)),
                            '\n    neighborhood_features  - ', tf.shape(neighborhood_features), '  -  ',
                            'Min : ', tf.reduce_min(neighborhood_features), ' / ',
                            'Max : ', tf.reduce_max(neighborhood_features), ' / ',
                            'Mean : ', tf.reduce_mean(neighborhood_features), ' / ',
                            'Num > 1e6 : ', tf.reduce_any(tf.greater(tf.abs(neighborhood_features), 1e6)),
                            tf.reduce_sum(tf.cast(tf.greater(tf.abs(neighborhood_features), 1e6), tf.int32)),
                            '\n        weighted_features  - ', tf.shape(weighted_features), '  -  ',
                            'Min : ', tf.reduce_min(weighted_features), ' / ',
                            'Max : ', tf.reduce_max(weighted_features), ' / ',
                            'Mean : ', tf.reduce_mean(weighted_features), ' / ',
                            'Num > 1e6 : ', tf.reduce_any(tf.greater(tf.abs(weighted_features), 1e6)),
                            tf.reduce_sum(tf.cast(tf.greater(tf.abs(weighted_features), 1e6), tf.int32)),
                            '\n             Out features  - ', tf.shape(output_features), '  -  ',
                            'Min : ', tf.reduce_min(output_features), ' / ',
                            'Max : ', tf.reduce_max(output_features), ' / ',
                            'Mean : ', tf.reduce_mean(output_features), ' / ',
                            'Num > 1e6 : ', tf.reduce_any(tf.greater(tf.abs(output_features), 1e6)),
                            tf.reduce_sum(tf.cast(tf.greater(tf.abs(output_features), 1e6), tf.int32)),
                            summarize=5000,
                            output_stream=sys.stdout)

        ind = tf.argmax(tf.reduce_max(weighted_features, axis=(0, 2)))

        with tf.control_dependencies([print_op]):
            print_op2 = tf.print('\nResult at ', ind, '\n\n',
                                 all_weights[ind, :, :],
                                 '\n\n',
                                 neighborhood_features[ind, :, :],
                                 '\n\n',
                                 tf.transpose(weighted_features, [1, 0, 2])[ind, :, :],
                                 '\n',
                                 summarize=5000,
                                 output_stream=sys.stdout)
        ...
        with tf.control_dependencies([print_op2]):
            output_features = tf.identity(output_features)

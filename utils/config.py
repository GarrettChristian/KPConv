...
class MultiConfig:
    """
    Class containing the parameters you want to modify for multiple datasets
    """
    ...
    # Dataset scales. For each dataset, minimum and maximum values, followed by the scale of validation.
    scales = [[]]
    ...
    # Ratio between the input sphere radius and the first subsampling dl
    input_scale_ratio = 50.0
    ...
    def load(self, path):
        ...
        for line in lines:
            ...
            if len(line_info) > 1 and line_info[0] != '#':
                ...
                elif line_info[0] == 'datasets':
                    self.datasets = [b for b in line_info[2:]]

                elif line_info[0] == 'scales':
                    self.scales = [[]]
                    for b in line_info[2:-1]:
                        if b == ',':
                            self.scales += []
                        else:
                            self.scales[-1] += [float(b)]
    ...
    def save(self, path):
        ...
        with open(join(path, 'parameters.txt'), "w") as text_file:
            ...
            text_file.write('scales =')
            for aa in self.scales:
                for a in aa:
            ...
            text_file.write('first_subsampling_dl = {:.3f}\n'.format(self.first_subsampling_dl))
            text_file.write('input_scale_ratio = {:.3f}\n'.format(self.input_scale_ratio))
            text_file.write('num_kernel_points = {:d}\n'.format(self.num_kernel_points))
            text_file.write('density_parameter = {:.3f}\n'.format(self.density_parameter))
            text_file.write('fixed_kernel_points = {:s}\n'.format(self.fixed_kernel_points))
            text_file.write('KP_extent = {:.3f}\n'.format(self.KP_extent))
